...
-- remove any plugin mapping that clobbers <C-h> etc.
...
local function try_win_or_tmux(dir)
	...
	-- try to move in nvim: wincmd {h|j|k|l}
	vim.cmd("wincmd " .. dir)
	if vim.api.nvim_get_current_win() == cur then
		-- still same window -> try tmux (only if inside tmux)
		if vim.env.TMUX then
			-- use tmux select-pane so it doesn't rely on plugin
